...
class Rab extends CI_Controller{
  ...
  function rab_bulanan(){
    ...
    foreach ($data as $vw) {
      ...
      $no +=1;
      $output['data'][] = array($vw->no_rab,$vw->tgl_rab,'<span class="box-sts" style="background:'.$bg.'">'.$vw->sts_rab.'</span>',$vw->jenis_rab,'Rp '. number_format($vw->total_pengeluaran , 0, ',', '.'),'<a href="'.base_url('Admin/Form/rab?no_rab='.$vw->no_rab.'&id_rab='.$vw->id_rab).
      '" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i> Tinjauan</a>');
    }
